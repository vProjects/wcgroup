...
class manageusers{
	...
	*/
	...
	function addUserInfo($table_name,$user_id,$title,$f_name,$m_name,$l_name,$dob,$father_name,$gender,$mar_status,$occupation,$emp_name,$bsns_name,$designation,$duties,$pan_no ,$house_no,$building_name,$road,$area,$landmark,$city,$state,$pincode,$tel_no,$mobile,$email_id){
	...
	*/
	...
	/*
	* Function use to fetch clients basic Info
	* @returns newslettes
	*/
	
	function showClients($values){
		if($values=='all')
			$query = $this->link->prepare("select * from basic_information");
		else if($values=='firstName')
			$query = $this->link->prepare("select f_name from basic_information");
		$query->execute();
